...
namespace MinesweeperSolverDemo.Lib.Solver
{
    ...
    {
        //static int solverType = 1;
        static int solverType = 2;

        public static SingleGameSolver createSolver(Random rand, Visualizer vis, GameBoard board)
        {
            if (solverType == 1)
                return new SingleGameSolver(rand, vis, board);
            if (solverType == 2)
                return new ModelCheckingSolver(rand, vis, board);
            return null;
        }

        public static SingleGameSolver createSolver(Random rand, Visualizer vis)
        {
            if (solverType == 1)
                return new SingleGameSolver(rand, vis);
            if (solverType == 2)
                return new ModelCheckingSolver(rand, vis);
            return null;
        }
    }
