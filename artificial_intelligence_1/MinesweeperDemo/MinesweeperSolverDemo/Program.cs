...
namespace MinesweeperSolverDemo
{
    public class Program
    {
        ...
        {
            Task<bool> runFormTask = new Task<bool>(() => openForm());
            runFormTask.Start();
        ...
        private static void PlayGame()
        {
            ...
            SingleGameSolver solver = new ModelCheckingSolver(rand, vis);
            while(input != 'Q')
            {
                if (solver == null)
                {
                    solver = EngineFactory.createSolver(rand, vis);
                    PlayCommands();
                }
                if (input == 'A')
                {
