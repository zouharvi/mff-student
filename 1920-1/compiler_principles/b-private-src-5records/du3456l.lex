...
'([^\n']|'')*' {
    ...
    mlc::dropApostrophe(string_literal);
    return parser::make_STRING(ctx->tab->ls_str().add(string_literal), ctx->curline);
}

'([^\n']|'')* {
    mlc::dropApostrophe(string_literal = yytext + 1);
    BEGIN(CORRUPTED_STR);
}
...
{IDENT} {
    ...
    return parser::make_IDENTIFIER(ctx->tab->ls_id().add(mlc::upper(string_literal)), ctx->curline);
}

{UINT} {
    return parser::make_UINT(ctx->tab->ls_int().add(mlc::convert_int(yytext, ctx->curline)), ctx->curline);
}

{UINT}(\.{UINT})[Ee][+-]?{IDENT}? {
    mlc::message(mlc::DUERR_BADREAL, ctx->curline, yytext);
    return parser::make_REAL(ctx->tab->ls_real().add(mlc::convert_real(yytext)), ctx->curline);
}
