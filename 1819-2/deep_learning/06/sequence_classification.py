    ...
    def __init__(self, args):
        ...
        if args.hidden_layer is not None:
            last_layer = tf.layers.dense(
                last_layer, args.hidden_layer, tf.nn.relu)

        # TODO: Generate predictions using a fully connected layer
        # with one output and `tf.nn.sigmoid` activation.
        predictions = tf.layers.dense(last_layer, 1, activation=tf.nn.sigmoid)
        ...
        tf.summary.create_file_writer(flush_millis=10*1000)
